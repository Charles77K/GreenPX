...
import {FOUNDERS, VALUES_CARD_CONTENT, WHY_CHOOSE_US} from "@/components/about/static";
      ...
      <div className="bg-brandFadeBlue py-16 md:py-28 mt-5 px-4">
        <section className="flex flex-col md:flex-row gap-10 items-center justify-center responsive-container">
          ...
          <div className="flex flex-col items-start gap-4">
            ...
            <div>
            <Line color="bg-brandGreen" />
            <h2 className="text-brandGreen text-base font-medium">Our Mission</h2>
            </div>
            <h1 className="text-black text-2xl md:text-4xl 2xl:text-5xl max-w-sm font-bold">Lighting The Future With Clean Energy</h1>
            <p className={"text-brandGray text-xs md:text-sm max-w-md"}>At Green Potentia, we make clean energy simple and accessible. Our
              affordable and reliable solar solutions power homes, businesses, and
              communities, reducing reliance on fossil fuels. With a focus on
              innovation and sustainability, we’re building a future where everyone
              enjoys uninterrupted, eco-friendly electricity.</p>
          </div>
          {/*energy bulb image */}
          <div>
            <Image src={'/about/energy.png'} alt={'energy-bulb'} width={500} height={500} className={"rounded-md"}/>
          </div>
        </section>
      </div>
    ...
    {/*  why choose us? */}
      <div className={"bg-brandGreen py-10 px-4 md:py-16"}>
        <section className="flex-col-center gap-10 responsive-container">
          <h1 className={"text-3xl md:text-5xl max-w-md 2xl:text-6xl text-center text-white font-semibold"}>Why Choose Green Potentia?</h1>
          {/*  cards */}
          <div className={"grid grid-cols-1 xs:grid-cols-2 gap-5 md:grid-cols-4"}>
    ...
    {/*  the minds behind the mission : founder*/}
      <div className={"py-16 px-4"}>
        <section className={"flex-col-center"}>
          ...
          <h2 className="text-3xl md:text-4xl 2xl:text-5xl text-center text-brandBlack max-w-md font-semibold">The Minds Behind The <GreenSpan>Mission</GreenSpan></h2>
          <p className={"text-brandGray text-sm md:text-base max-w-xl text-center mt-3"}>We’re a team of renewable energy experts, designers, strategists, and tech innovators united by one goal: delivering sustainable energy solutions that power lives and grow economies</p>
